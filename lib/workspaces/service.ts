...
export async function listUserWorkspaces(userId: number): Promise<Workspace[]> {
  ...
  return ((await db.query<Workspace & { created_at: number; allow_platform_channels: number; display_currency: string | null; display_symbol: string | null; display_rate: number | null }, [number]>(
...
export async function createWorkspace(input: { userId: number; name: string; kind?: "personal" | "team" }): Promise<Workspace> {
  ...
  return db.transaction(async () => {
    const row = ((await db.query<{ id: number }, [string, string, number, number | null, number]>(
    ...
    ((await db.query("INSERT INTO workspace_members (workspace_id, user_id, role, created_at) VALUES (?, ?, 'owner', ?)").run(row.id, input.userId, now)));
    ((await db.query("INSERT INTO wallets (workspace_id, balance_units) VALUES (?, 0)").run(row.id)));
...
export async function ensurePersonalWorkspace(userId: number, name: string): Promise<Workspace> {
  const db = await getDatabase();
  const existing = ((await db.query<Workspace & { created_at: number; allow_platform_channels: number; display_currency: string | null; display_symbol: string | null; display_rate: number | null }, [number, number]>(
    `SELECT w.id, w.kind, w.name, w.status, w.timezone, w.allow_platform_channels, w.display_currency, w.display_symbol, w.display_rate, m.role, w.created_at
     FROM workspaces w JOIN workspace_members m ON m.workspace_id = w.id
  ...
  return (await createWorkspace({ userId, name, kind: "personal" }));
}

export async function acceptWorkspaceInvite(
  input: { userId: number; email: string; token: string },
  database?: AsyncSqliteQueryAdapter,
  ...
  return db.transaction(async () => {
    const invite = ((await db.query<{ id: number; workspace_id: number; email: string; role: "admin" | "member" }, [string, number]>(
