...
export async function requireWorkspacePermission(userId: number, workspaceId: number, action: WorkspaceAction): Promise<Workspace> {
  ...
  const row = db.query<Workspace & { created_at: number; allow_platform_channels: number; display_currency: string | null; display_symbol: string | null; display_rate: number | null }, [number, number]>(
