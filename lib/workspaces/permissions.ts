...
export async function requireWorkspacePermission(userId: number, workspaceId: number, action: WorkspaceAction): Promise<Workspace> {
  ...
  const row = db.query<Workspace & { created_at: number; allow_platform_channels: number }, [number, number]>(
  ...
  const { created_at, allow_platform_channels, ...workspace } = row;
  return { ...workspace, allowPlatformChannels: allow_platform_channels !== 0, createdAt: created_at };
}
