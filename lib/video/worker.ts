...
export async function pollVideoTask(registry: RelayRegistry, task: VideoTask): Promise<VideoTask> {
  ...
  try {
    ...
    const response = await fetch(`${channel.baseUrl.replace(/\/+$/, "")}${statusPath}`, { headers: headersFor(channel, key), signal: AbortSignal.timeout((await registry.getSettings()).requestTimeoutMs) });
    ...
    if (url || ["succeed", "succeeded", "completed", "success"].includes(status)) {
      ...
      const next = (await registry.updateVideoTask(task.id, { state: "failed", error: typeof result.message === "string" ? result.message : "video provider reported failure", nextPollAt: null })) ?? task; await recordUsage(registry, next, false, response.status); return next;
