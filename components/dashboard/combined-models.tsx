...
export function CombinedModels({ workspaceId, locale, canManage }: { workspaceId: string; locale: Locale; canManage: boolean }) {
  ...
  return <section className="mt-8 border-t border-border pt-7">
    ...
    {editing === null ? <ul className="list mt-4 divide-y divide-border border-y border-border">
    ...
    </ul> : <form onSubmit={save} className="mt-5 space-y-5">
      ...
      <div className="flex gap-2"><button type="submit" className="btn btn-primary" disabled={busy}>{busy ? t("保存中…", "Saving…") : t("保存组合", "Save combination")}</button><button type="button" className="btn" disabled={busy} onClick={() => setEditing(null)}>{t("取消", "Cancel")}</button></div>
    </form>}
  </section>;
}
