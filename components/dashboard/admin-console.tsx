...
export function AdminConsole({ locale, section = "overview" }: { locale: Locale; section?: "overview" | "channels" | "groups" }) {
  ...
  async function submitGroup(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (saved) { setGroupDraft(BLANK_GROUP); setGroupDialogOpen(false); }
  }
  ...
  return <div className="flex flex-col gap-6">
    ...
    {snapshot && section === "groups" && <div className="space-y-5">
      <div className="flex justify-end">
        <Dialog open={groupDialogOpen} onOpenChange={(open) => { if (!busy) { setGroupDialogOpen(open); if (!open) setGroupDraft(BLANK_GROUP); } }}>
          <DialogTrigger asChild>
            <Button onClick={() => { setGroupDraft(BLANK_GROUP); setMutationError(""); }}><Plus className="size-4" />{t("Add group", "新增分组")}</Button>
          </DialogTrigger>
          <DialogContent closeLabel={t("Close", "关闭")}>
            <DialogHeader>
              <DialogTitle>{groupDraft.id === null ? t("Add group", "新增分组") : t(`Edit group “${groupDraft.name}”`, `编辑分组“${groupDraft.name}”`)}</DialogTitle>
              <DialogDescription>{t("A group decides which channels serve a request and which fee ratio applies. Its name is a permanent identifier used by channels and API keys.", "分组决定请求由哪些渠道服务、适用什么倍率。分组名是不可变标识，渠道与密钥按名引用。")}</DialogDescription>
            </DialogHeader>
            <form onSubmit={submitGroup} className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2"><Field name="group-description" title={t("Description", "说明")}>
              ...
              {mutationError && <p role="alert" className="sm:col-span-2 text-sm text-destructive">{mutationError}</p>}
              <p className="text-xs text-muted-foreground sm:col-span-2">{t("1–32 characters, without spaces or commas. Keep the name used by upstream channels.", "1–32 个字符，不含空格和逗号。请沿用上游渠道引用的分组名（可为中文）。")}</p>
              <div className="flex justify-end gap-2 sm:col-span-2">
                <Button type="button" variant="outline" onClick={() => { setGroupDialogOpen(false); setGroupDraft(BLANK_GROUP); }} disabled={busy}>{t("Cancel", "取消")}</Button>
        ...
        <TableBody>{registeredGroups.map((item) => {
          ...
          return <TableRow key={item.id}>
            ...
            <TableCell><div className="flex gap-2">
              <Button size="sm" variant="outline" disabled={disabled} onClick={() => { setGroupDraft({ id: item.id, name: item.name, displayName: item.displayName, ratio: String(item.ratio), description: item.description, status: item.status }); setMutationError(""); setGroupDialogOpen(true); }}>{t("Edit", "编辑")}</Button>
      ...
      {unregisteredGroups.length > 0 && <div className="rounded-md border border-border bg-card p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">{unregisteredGroups.map((name) => <button key={name} type="button" className="badge badge-outline font-mono" onClick={() => { setGroupDraft({ ...BLANK_GROUP, name }); setMutationError(""); setGroupDialogOpen(true); }}>{name}</button>)}</div>
      </div>}
