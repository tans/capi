...
export function ChannelActions({workspaceId,channelId,status,name}:{workspaceId:number;channelId:number;status:number;name:string}) {
 ...
 return <div className="flex flex-wrap items-center gap-1"><button className="btn btn-xs btn-ghost" onClick={()=>setEditing(true)}>Edit</button><button className="btn btn-xs btn-ghost" onClick={()=>act("PATCH",{status:status===1?2:1})}>{status===1?"Disable":"Enable"}</button><button className="btn btn-xs btn-ghost text-error" onClick={()=>{if(confirm("Delete this channel?"))act("DELETE")}}>Delete</button>{error&&<span className="text-xs text-error">{error}</span>}</div>
}
