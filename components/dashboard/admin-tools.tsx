...
type User = { id: number; email: string; name: string; role: "user" | "admin"; balance: number; spent: number };
type Code = { id: number; code: string; amount: number; redeemed_by: number | null; redeemed_at: number | null; expires_at: number | null; redeemed_by_email: string | null; redeemed_by_name: string | null; workspace_id: number | null; workspace_name: string | null; status: "available" | "expired" | "redeemed" };
...
export function AdminTools({ locale, section }: { locale: "zh" | "en"; section: Section }) {
  ...
  async function createCode(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(`${t("Created code", "兑换码已创建")}: ${result.code}`); setExpiresAt(""); void load();
  ...
  return <section className="flex flex-col gap-5">
    ...
    {!loading && section === "codes" && <><form onSubmit={createCode} className="flex max-w-2xl flex-col gap-3 rounded-box border border-border bg-card p-4 sm:flex-row sm:items-end"><label className="form-control"><span className="label-text mb-2 text-sm">{t("Amount (USD)", "金额（美元）")}</span><Input required aria-label={t("Amount", "金额")} type="number" min="0.01" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} disabled={creating} /></label><label className="form-control"><span className="label-text mb-2 text-sm">{t("Expires (optional)", "有效期（可选）")}</span><Input aria-label={t("Expires", "有效期")} type="datetime-local" value={expiresAt} onChange={(event) => setExpiresAt(event.target.value)} disabled={creating} /></label><Button type="submit" disabled={creating}>{creating ? t("Creating…", "创建中…") : t("Create code", "创建兑换码")}</Button></form><div className="overflow-x-auto rounded-box border border-border bg-card"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="p-3">{t("Code", "兑换码")}</th><th className="p-3">{t("Amount", "金额")}</th><th className="p-3">{t("Status", "状态")}</th><th className="p-3">{t("Destination / redeemer", "到账空间 / 兑换人")}</th><th className="p-3">{t("Redeemed at", "兑换时间")}</th></tr></thead><tbody>{codes.map((code) => <tr className="border-b last:border-0" key={code.id}><td className="p-3 font-mono">{code.code}</td><td className="p-3">${code.amount.toFixed(2)}</td><td className="p-3">{code.status === "redeemed" ? t("Redeemed", "已兑换") : code.status === "expired" ? t("Expired", "已过期") : t("Available", "可兑换")}</td><td className="p-3">{code.workspace_name || "—"}<br /><span className="text-xs text-muted-foreground">{code.redeemed_by_name || code.redeemed_by_email || "—"}</span></td><td className="p-3 text-muted-foreground">{code.redeemed_at ? new Date(code.redeemed_at).toLocaleString() : "—"}</td></tr>)}</tbody></table></div></>}
  </section>;
}
