...
export default function NewWorkspacePage() {
  ...
  return <div className="mx-auto max-w-xl"><a className="link link-hover text-sm" href={localeHref(locale,"/dashboard")}>← Dashboard</a><div className="card mt-5 border border-border bg-card"><div className="card-body"><h1 className="card-title text-xl">Create team workspace</h1><p className="text-sm text-muted-foreground">Create a shared space for your team, keys and usage.</p><form className="mt-5 flex flex-col gap-4" onSubmit={submit}><label className="form-control"><span className="label-text mb-2 text-sm font-medium">Workspace name</span><input className="input input-bordered w-full" value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Acme team" maxLength={100} required /></label>{error && <div className="alert alert-error py-2 text-sm">{error}</div>}<button className="btn btn-primary self-start" disabled={saving}>{saving ? "Creating…" : "Create team"}</button></form></div></div></div>;
}
