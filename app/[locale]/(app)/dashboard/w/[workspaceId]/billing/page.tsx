...
export default async function WorkspaceBilling({ params }: { params: Promise<{ locale: string; workspaceId: string }> }) {
  ...
  return <div className="flex flex-col gap-6">
    <div><a className="link link-hover text-sm" href={localeHref(locale, `/dashboard/w/${id}`)}>← {workspace.name}</a><h1 className="mt-3 text-2xl font-semibold">Billing</h1><p className="mt-1 text-sm text-muted-foreground">Credit is added to: <strong>{workspace.name}</strong></p></div>
    <div className="grid gap-4 sm:grid-cols-3">{[["Available", rows.balance_units - rows.reserved_units], ["Balance", rows.balance_units], ["Reserved", rows.reserved_units]].map(([label, value]) => <div className="stat rounded-box border border-border bg-card" key={label as string}><div className="stat-title">{label}</div><div className="stat-value text-2xl">${(Number(value) / 500000).toFixed(2)}</div><div className="stat-desc">{rows.currency}</div></div>)}</div>
    <RedeemCodeForm workspaceId={id} workspaceName={workspace.name} />
    <div className="rounded-box border border-border bg-card"><div className="border-b border-border p-4 font-medium">Recent activity</div><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="p-3">Date</th><th className="p-3">Description</th><th className="p-3 text-right">Amount</th></tr></thead><tbody>{entries.map((entry, index) => <tr className="border-b last:border-0" key={`${entry.created_at}-${index}`}><td className="p-3 text-muted-foreground">{new Date(entry.created_at).toLocaleString()}</td><td className="p-3">{entry.reason}</td><td className={`p-3 text-right font-medium ${entry.delta_units >= 0 ? "text-success" : ""}`}>{entry.delta_units >= 0 ? "+" : ""}${(entry.delta_units / 500000).toFixed(2)}</td></tr>)}</tbody></table></div></div>
  </div>;
}
