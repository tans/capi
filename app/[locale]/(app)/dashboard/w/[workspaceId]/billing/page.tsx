...
export default async function WorkspaceBilling({ params }: { params: Promise<{ locale:string; workspaceId:string }> }) {
 ...
 const response=await fetch(new URL(`/api/workspaces/${id}/billing`, process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"), { cache:"no-store" }).catch(()=>null); const payload=response?.ok ? await response.json() : null; const rows=payload?.data ?? payload ?? {};
 return <div className="flex flex-col gap-6"><div><a className="link link-hover text-sm" href={localeHref(locale,`/dashboard/w/${id}`)}>← {workspace.name}</a><h1 className="mt-3 text-2xl font-semibold">Billing</h1></div><div className="card border border-border bg-card"><div className="card-body"><pre className="overflow-auto text-xs text-muted-foreground">{JSON.stringify(rows,null,2)}</pre></div></div></div>;
}
