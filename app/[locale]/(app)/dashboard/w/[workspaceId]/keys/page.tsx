...
import { WorkspaceKeyActions } from "@/components/dashboard/workspace-key-actions"; import { WorkspaceKeyManager } from "@/components/dashboard/workspace-key-manager"; import { resolveLocale } from "@/lib/i18n/server";
...
export default async function WorkspaceKeys({ params }: { params: Promise<{ locale:string; workspaceId:string }> }) {
 ...
 return <div className="flex flex-col gap-6"><div><a className="link link-hover text-sm" href={localeHref(locale,`/dashboard/w/${id}`)}>← {workspace.name}</a><h1 className="mt-3 text-2xl font-semibold">API keys</h1></div><WorkspaceKeyManager workspaceId={id} canManage={workspace.role !== "member"}/><div className="overflow-x-auto rounded-box border border-border bg-card"><table className="table"><thead><tr><th>Prefix</th><th>Project</th><th>Status</th><th></th></tr></thead><tbody>{rows.map((key) => <tr key={key.id}><td className="font-mono">{key.name || `Key ${key.id}`}</td><td>{key.projectId ?? "Default"}</td><td><span className="badge badge-success badge-outline">Active</span></td><td>{workspace.role !== "member" && <WorkspaceKeyActions workspaceId={id} keyId={key.id}/>}</td></tr>)}{rows.length===0&&<tr><td colSpan={3} className="py-10 text-center text-sm text-muted-foreground">No workspace keys yet.</td></tr>}</tbody></table></div></div>;
}
