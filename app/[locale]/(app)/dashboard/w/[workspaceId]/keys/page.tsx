  ...
  return <div className="flex flex-col gap-6">
    <div>
      ...
      <div className="mt-3 flex flex-wrap items-end justify-between gap-3">
        ...
        {wallet && <div className="rounded-box border border-base-300 bg-base-100 px-4 py-2 text-sm"><span className="text-base-content/60">{t.wallet}</span> <span className="font-medium">${(wallet.balanceUnits / 500_000).toFixed(2)}</span></div>}
      </div>
    </div>
    <WorkspaceKeyManager workspaceId={id} groups={groups} canManage={canManage} locale={locale} />
    <WorkspaceKeyTable workspaceId={id} keys={keys} groups={groups} canManage={canManage} locale={locale} />
  </div>;
}
