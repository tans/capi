...
export async function PATCH(request: Request, { params }: { params: Promise<{ wid: string; mid: string }> }) {
  return authResponse(async () => {
    ...
    if (body.action === "transfer_owner") {
      ...
      db.transaction(() => { db.query("UPDATE workspace_members SET role='admin' WHERE id=?").run(current.id); db.query("UPDATE workspace_members SET role='owner' WHERE id=?").run(target.id); }).immediate();
      return Response.json({ ok: true });
    }
    if (body.role !== "admin" && body.role !== "member") return Response.json({ error: "invalid role" }, { status: 400 });
    const target = db.query<{ role: string }, [number, number]>("SELECT role FROM workspace_members WHERE id=? AND workspace_id=? AND status='active'").get(mid, wid);
    if (!target) return Response.json({ error: "member not found" }, { status: 404 });
    if (target.role === "owner") return Response.json({ error: "owner role cannot be changed" }, { status: 409 });
    db.query("UPDATE workspace_members SET role=? WHERE id=? AND workspace_id=?").run(body.role, mid, wid);
    return Response.json({ ok: true });
  });
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ wid: string; mid: string }> }) {
  return authResponse(async () => {
    ...
    const db = await getDatabase();
    const target = db.query<{ role: string; user_id: number }, [number, number]>("SELECT role,user_id FROM workspace_members WHERE id=? AND workspace_id=? AND status='active'").get(mid, wid);
    if (!target) return Response.json({ error: "member not found" }, { status: 404 });
    if (target.role === "owner") return Response.json({ error: "owner cannot be removed" }, { status: 409 });
    db.transaction(() => {
      db.query("UPDATE workspace_members SET status='removed' WHERE id=? AND workspace_id=?").run(mid, wid);
      db.query("UPDATE api_keys SET config=json_set(config, '$.status', 2) WHERE workspace_id=? AND user_id=? AND json_extract(config, '$.status')=1").run(wid, target.user_id);
    }).immediate();
    return Response.json({ ok: true });
  });
}
