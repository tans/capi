...
export async function GET(request: Request, { params }: { params: Promise<{ wid: string }> }) {
  return authResponse(async () => {
    ...
    const invites = workspace.role === "member" ? [] : (await db.query<{id:number;email:string;role:string;expiresAt:number;createdAt:number}, [number]>("SELECT id,email,role,expires_at AS expiresAt,created_at AS createdAt FROM workspace_invites WHERE workspace_id=? AND accepted_at IS NULL AND revoked_at IS NULL AND expires_at>? ORDER BY id DESC").all(id));
    const data = (await db.query<{ id: number; userId: number; email: string; name: string; role: string; status: string }, [number]>(
...
export async function POST(request: Request, { params }: { params: Promise<{ wid: string }> }) {
  return authResponse(async () => {
    ...
    const existingMember = (await db.query<{id:number}, [number,string]>("SELECT m.id FROM workspace_members m JOIN users u ON u.id=m.user_id WHERE m.workspace_id=? AND lower(u.email)=lower(?) AND m.status=\'active\'").get(id,email));
    if (existingMember) return Response.json({ error: "user is already a workspace member" }, { status: 409 });
    const pending = (await db.query<{id:number}, [number,string,number]>("SELECT id FROM workspace_invites WHERE workspace_id=? AND lower(email)=lower(?) AND accepted_at IS NULL AND revoked_at IS NULL AND expires_at>? LIMIT 1").get(id,email,Date.now()));
    if (pending) return Response.json({ error: "an active invite already exists" }, { status: 409 });
    const token=randomBytes(32).toString("base64url"); const hash=createHash("sha256").update(token).digest("hex"); const invite=(await db.query<{id:number},[number,string,string,string,number,number]>("INSERT INTO workspace_invites (workspace_id,email,token_hash,role,expires_at,created_at) VALUES (?, ?, ?, ?, ?, ?) RETURNING id").get(id,email,hash,requestedRole,Date.now()+7*86400000,Date.now()));
