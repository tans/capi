...
export async function GET(request: Request, context: Context) {
  return authResponse(async () => {
    ...
    const allowPlatform = registry.workspaceAllowsPlatformChannels(workspaceId);
    const availableModels = [...new Set(registry.listChannels()
...
export async function PUT(request: Request, context: Context) {
  return authResponse(async () => {
    ...
    const names = [...aliases.filter((alias) => alias !== previousName), ...registry.listChannels().flatMap((channel) => channel.models)];
    const result = validateCombinedModel(body, names, aliases);
    if (!result.ok) return Response.json({ error: result.error }, { status: 400 });
    const available = new Set(registry.listChannels()
      .filter((channel) => isChannelAccessible(channel, workspaceId, registry.workspaceAllowsPlatformChannels(workspaceId)))
    ...
    db.transaction(() => {
      if (previousName) db.query("DELETE FROM combined_models WHERE workspace_id = ? AND name = ?").run(workspaceId, previousName);
...
export async function DELETE(request: Request, context: Context) {
  return authResponse(async () => {
    ...
    const db = await getDatabase();
    db.query("DELETE FROM combined_models WHERE workspace_id = ? AND name = ?").run(workspaceId, name);
