...
export async function GET(request: Request, { params }: { params: Promise<{ wid: string }> }) {
  return authResponse(async () => {
    ...
    const incidents = (await registry.listSecurityIncidents(workspaceId, { limit: Number(url.searchParams.get("limit") || 50), status: url.searchParams.get("status") || undefined }));
...
export async function PATCH(request: Request, { params }: { params: Promise<{ wid: string }> }) {
  return authResponse(async () => {
    ...
    const registry = await getRegistry();
    if (!(await registry.updateSecurityIncident(body.id as number, workspaceId, body.status, user.id, body.severity))) return Response.json({ error: "incident not found or status invalid" }, { status: 404 });
