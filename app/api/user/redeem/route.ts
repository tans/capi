...
export async function POST(request: Request) {
  return authResponse(async () => {
    ...
    const result = await db.transaction(async () => {
      const row = (await db.query<{ id: number; amount_quota: number; expires_at: number | null; redeemed_by: number | null }, [string]>(
        "SELECT id, amount_quota, expires_at, redeemed_by FROM redeem_codes WHERE code = ?",
      ).get(code));
      if (!row) return null;
      if (row.redeemed_by !== null) {
        const previous = (await db.query<{ amount_quota: number }, [number, number, number]>(
      ...
      const redeemed = (await db.query("UPDATE redeem_codes SET redeemed_by = ?, redeemed_at = ? WHERE id = ? AND redeemed_by IS NULL").run(user.id, Date.now(), row.id));
      if (redeemed.changes !== 1) return null;
      const wallet = (await db.query<{ workspace_id: number }, [number]>("SELECT workspace_id FROM wallets WHERE workspace_id = ?").get(target));
      if (!wallet) throw new Error("wallet not found");
      const entry = (await db.query<{ id: number }, [number, number, string, number, number]>(
      ...
      const updated = (await db.query("UPDATE wallets SET balance_units = balance_units + ? WHERE workspace_id = ?").run(row.amount_quota, target));
      if (updated.changes !== 1) throw new Error("wallet update failed");
      (await db.query("INSERT INTO redeem_code_credits (redeem_code_id, workspace_id, wallet_entry_id) VALUES (?, ?, ?)").run(row.id, target, entry.id));
    ...
    const finalCurrency = (await workspaceCurrency(db, target, systemCurrency((await registry.getSettings()))));
