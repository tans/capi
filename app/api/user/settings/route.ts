...
export async function GET(request: Request) {
  ...
  const config = row ? JSON.parse(row.config) : { accountName: user.name, accountEmail: user.email, notifications: {} };
...
export async function PUT(request: Request) {
  ...
  const config = { accountName: body.accountName?.trim() || user.name, accountEmail, notifications: body.notifications ?? {} }; const savedAt = Date.now(); const db = await setup();
